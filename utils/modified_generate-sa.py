...
def add_strands (mynewpositions, mynewa1s, mynewa3s):
    '''
    Populates positions, a1s, a3s, with p from their respective mynewpos, mynewa1s, 
    mynewa3s arrays
...
class StrandGenerator (object):
    ...
    def generate_sticky(self, top_bp, bot_bp, bot_seq_start_nn, start_pos=np.array([0, 0, 0]), dir=np.array([0, 0, 1]), perp=False, rot=0.):
        ...
        # ortho vector to the dir respect to top strand
        dir_norm = np.sqrt(np.dot(dir, dir))
        if dir_norm < 1e-10:
...
def read_strands(filename):
    ...
    In the above example, the sticky ends are TTTT and AAAA (they must match in length)
    The code will automatically generate the complements after the sticky end of the first strand
