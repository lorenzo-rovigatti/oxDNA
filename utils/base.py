...
class Logger(object):
    ...
    def log(msg, level=None, additional=None):
        if level is None: 
            level = Logger.INFO
        # Ensure debug_level is always an integer
        if Logger.debug_level is None:
...
class System(object):
    ...
    def __init__(self, box, time=0, E_pot=0, E_kin=0):
        ...
        self._N_cells = np.array(np.floor (self._box / 3.), int)
        for kk in [0, 1, 2]:
    ...
    def do_cells (self):
        ...
        self._head = [False,] * int(self._N_cells[0] * self._N_cells[1] * self._N_cells[2])
        for n in self._nucleotides:
            cs = np.array((np.floor((n.cm_pos/self._box - np.rint(n.cm_pos / self._box ) + 0.5) * (1. - FLT_EPSILON) * self._box / self._cellsides)), int)
    ...
    def is_overlapping_better(self, s):
        ...
        for n1 in s._nucleotides:
            # trovo la cella
            cs = np.array(np.floor((n1.cm_pos/self._box - np.rint(n1.cm_pos / self._box ) + 0.5) * (1.-FLT_EPSILON) * self._box / self._cellsides), int)
            cella = cs[0] + self._N_cells[0] * cs[1] + self._N_cells[0] * self._N_cells[1] * cs[2]
