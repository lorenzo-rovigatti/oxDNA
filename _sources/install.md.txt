...
When `cmake` looks for Python executables, it searches first for `python`, and if that's not found, for `python3`. `make install` then runs `python(3) -m pip install` to install `oxpy` and `oat` with the Python executable found by Cmake. If the `-DOxpySystemInstall=On` option is not chosen, the `pip` command will use the `--user` flag, which will (with default settings) install the two libraries in `$HOME/.local/bin/`, which is not on the `$PATH`. If your default Python is something you installed (e.g via `conda`), `-DOxpySystemInstall=On` is necessary to correctly install the Python libraries. On shared systems (e.g HPC systems), it should not be set, as you may not have write access to the shared Python's `site-packages` directory.
