...
public:
	...
	double get_seconds () { return (double) _time / (double) CPS; }

	/// returns the time passed, converted in seconds, as a float
	float get_secondsf () { return (float) get_seconds(); }

	/// returns the description of the clock
	std::string get_desc () { return _desc; }
...
public:
	...
	Timer * get_timer_by_desc (std::string desc) { return _desc_map[desc]; } 
