...
CUDABaseThermostat<number, number4> *CUDAThermostatFactory::make_thermostat(input_file &inp, BaseBox<number> * box) {
	...
	if(!strncmp(thermostat_type, "john", 512)) return new CUDABrownianThermostat<number, number4>();
		...
		return new CUDASRDThermostat<number, number4>(box);
	}
...
template CUDABaseThermostat<float, float4> *CUDAThermostatFactory::make_thermostat<float, float4>(input_file &inp, BaseBox<float> * box);
template CUDABaseThermostat<double, LR_double4> *CUDAThermostatFactory::make_thermostat<double, LR_double4>(input_file &inp, BaseBox<double> * box);
