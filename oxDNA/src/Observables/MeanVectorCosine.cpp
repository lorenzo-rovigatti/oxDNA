...
void MeanVectorCosine<number>::init(ConfigInfo<number> &config_info) {
	...
	if (_first_particle_position > _last_particle_position && _last_particle_position > -1)
		throw oxDNAException("MeanVectorCosine: first_particle_position must be equal or less than last_particle_position, but they are respectively %d and %d.",_first_particle_position,_last_particle_position);	
...
void MeanVectorCosine<number>::get_settings(input_file &my_inp, input_file &sim_inp) {
	...
	if (getInputInt(&my_inp,"first_particle_position",&tmp,0) == KEY_FOUND) {
		...
		if (_first_particle_position < 0)
			throw oxDNAException("MeanVectorCosine, first_particle_position must be 0 or positive.");
	}
	...
	if (getInputInt(&my_inp,"last_particle_position",&tmp,0) == KEY_FOUND) {
		...
		if (_last_particle_position < 0)
			throw oxDNAException("MeanVectorCosine, last_particle_position must be 0 or positive.");
	}
...
//		printf("average %lf\n",average*_one_over_number_of_values);
	if(_vector_to_average == -1){
		...
		result  =  Utils::sformat("%14.14lf",cos(average));
	}	
	else if (_vector_to_average == 3){
		result  =  Utils::sformat("%14.14lf", average/(*this->_config_info.N-2));
	}	
	else {
		result  =  Utils::sformat("%14.14lf", average/(*this->_config_info.N-2));
	}
...
void MeanVectorCosine<number>::set_first_last_particle_id(){
	...
	//printf("_first_particle_id = %d _last_particle_id = %d\n",_first_particle_id,_last_particle_id);
	 ...
	 printf("number_of_values %d\n",number_of_values);
		...
		if (p[i]->n5 == P_VIRTUAL ) {chain_counter++;printf("Chain_counter %d\n",chain_counter);}//when it finds a terminal particle, increment the chain counter.
	}
...
template class MeanVectorCosine<float>;
template class MeanVectorCosine<double>;
