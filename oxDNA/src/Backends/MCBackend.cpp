...
void MCBackend<number>::get_settings(input_file &inp) {
	...
	if(strncasecmp(tmp, "npt", 256) == 0) {
		...
		if (check == KEY_NOT_FOUND || strncasecmp(tmp, "cells", 256) != 0) throw oxDNAException ("NPT ensemble requires cells to handle interaction lists; set list_type=cells in the input file");
