...
void LJInteraction<number>::init() {
	...
	for(int i = 0; i < 3; i++) {
		...
		_E_cut[i] = 4. * _epsilon[i] * (pow(_sigma[i]/rcut, (number)12.) - pow(_sigma[i]/rcut, (number)6.));
...
void LJInteraction<number>::read_topology(int N, int *N_strands, BaseParticle<number> **particles) {
	if(!_is_ka_mixture) BaseInteraction<number, LJInteraction<number> >::read_topology(N, N_strands, particles);
	else {
		*N_strands = N;
