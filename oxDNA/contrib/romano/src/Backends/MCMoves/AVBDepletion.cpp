...
void AVBDepletion<number>::init () {
	...
	OX_LOG(Logger::LOG_INFO, "(AVBDepletion.cpp)                               max overlap volume=%g, effective T=%g", ovolume, 1./kteq);
}
...
template<typename number>
bool AVBDepletion<number>::are_close(BaseParticle<number> * pt, BaseParticle<number> * p) {
...
void AVBDepletion<number>::apply (llint curr_step) {
	...
	while (it != target_neighs.end()) {
		if (!are_close(pt, *it)) it = target_neighs.erase(it);
		else ++it;
	}
	...
	if (move_in) {
		...
		if (are_close (pt, p) == false) throw oxDNAException ("investigate...");
	}
	else {
