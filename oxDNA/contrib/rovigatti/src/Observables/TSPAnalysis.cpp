...
void TSPAnalysis<number>::init(ConfigInfo<number> &config_info) {
	...
	_stars.resize(_N_stars, TSP<number>(_is_SPB));
	...
	for(int i = 0; i < _N_stars;i++) {
		...
		if(n_patches > 0) {
			...
			for(it = _stars[i].patches.begin(); it != _stars[i].patches.end(); it++) {
				...
				LR_vector<number> rel_pos_1 = this->_config_info.box->min_image(patch.pos(), star_pos);
				...
				for(jt = it; jt != _stars[i].patches.end(); jt++) {
					if(it != jt) {
						...
						LR_vector<number> rel_pos_2 = this->_config_info.box->min_image(patch_2.pos(), star_pos);
...
template<typename number>
void TSP<number>::_compute_eigenvalues() {
	...
	for(int i = 0; i < na; i++) {
		...
		for(it = arms[i].begin(); it != arms[i].end(); it++) {
			TSPParticle<number> *p = *it;
			LR_vector<number> i_pos = _config_info->box->get_abs_pos(p) - com;
...
template<typename number>
void Patch<number>::done(BaseBox<number> *box) {
	if(empty()) return;
