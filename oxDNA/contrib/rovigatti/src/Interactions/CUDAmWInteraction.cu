...
template <typename number, typename number4>
__device__ void _particle_particle_interaction(number4 &ppos, number4 &qpos, number4 &F, cuda_mW_bond_list<number, number4> &bond_list, int q_idx, CUDABox<number, number4> *box) {
...
template <typename number, typename number4>
__global__ void mW_forces(number4 *poss, number4 *forces, number4 *forces_3body, CUDABox<number, number4> *box) {
	...
	for(int j = 0; j < MD_N[0]; j++) {
		if(j != IND) {
			number4 qpos = poss[j];
			_particle_particle_interaction<number, number4>(ppos, qpos, F, bond_list, j, box);
		}
	}
...
template <typename number, typename number4>
__global__ void mW_forces(number4 *poss, number4 *forces, int *matrix_neighs, int *number_neighs, number4 *forces_3body, CUDABox<number, number4> *box) {
	if(IND >= MD_N[0]) return;
	...
	for(int j = 0; j < num_neighs; j++) {
		...
		_particle_particle_interaction<number, number4>(ppos, qpos, F, bond_list, k_index, box);
	}
	_three_body(bond_list, F, forces, forces_3body);
	forces[IND] = F;
	//LR_atomicAddXYZ(forces + IND, F);
...
template<typename number, typename number4>
void CUDAmWInteraction<number, number4>::compute_forces(CUDABaseList<number, number4> *lists, number4 *d_poss, GPU_quat<number> *d_orientations, number4 *d_forces, number4 *d_torques, LR_bonds *d_bonds, CUDABox<number, number4> *d_box) {
	...
	if(_v_lists != NULL) {
		if(_v_lists->use_edge()) throw oxDNAException("use_edge unsupported by mWInteraction");
				...
				mW_forces<number, number4>
					...
					(d_poss, d_forces, _v_lists->_d_matrix_neighs, _v_lists->_d_number_neighs, _d_forces_3body, d_box);
