...
template <typename number, typename number4>
__device__ void _particle_particle_interaction(number4 &ppos, number4 &qpos, number4 &F, int *strand_ids, CUDABox<number, number4> *box) {
	int pbtype = get_particle_btype<number, number4>(ppos);
	int p_idx = get_particle_index<number, number4>(ppos);
	int qbtype = get_particle_btype<number, number4>(qpos);
	int q_idx = get_particle_index<number, number4>(qpos);
...
template <typename number, typename number4>
__device__ void _particle_all_bonded_interactions(number4 &ppos, LR_bonds &bs, number4 &F, number4 *poss, number4 *forces, CUDABox<number, number4> *box) {
	int pbtype = get_particle_btype<number, number4>(ppos);
	// backbone or hub
	if(pbtype == N_DUMMY || pbtype == P_HUB) {
		...
		if(has_n3) {
			...
			_particle_particle_bonded_interaction<number, number4>(ppos, qpos, F, box);
		}

		// backbone-base
		if(pbtype == N_DUMMY) {
			number4 qpos = poss[IND + 1];
			_particle_particle_bonded_interaction<number, number4>(ppos, qpos, F, box, true);
		}
...
template <typename number, typename number4>
__global__ void Starr_forces(number4 *poss, number4 *forces, LR_bonds *bonds, int *strand_ids, CUDABox<number, number4> *box) {
	...
	_particle_all_bonded_interactions<number, number4>(ppos, bs, F, poss, forces, box);

	for(int j = 0; j < MD_N[0]; j++) {
		if(j != IND && bs.n3 != j && bs.n5 != j) {
			number4 qpos = poss[j];
			_particle_particle_interaction<number, number4>(ppos, qpos, F, strand_ids, box);
		}
	}
...
template <typename number, typename number4>
__global__ void Starr_forces(number4 *poss, number4 *forces, int *matrix_neighs, int *number_neighs, LR_bonds *bonds, int *strand_ids, CUDABox<number, number4> *box) {
	if(IND >= MD_N[0]) return;
...
template <typename number, typename number4>
__global__ void hub_forces(number4 *poss, number4 *forces, number4 *n3_forces, number4 *n5_forces, int *hubs, hub_bonds *bonds, LR_bonds *n3n5, CUDABox<number, number4> *box) {
	...
	if(bs_hub.n3 == P_INVALID) {
		for(int an = 0; an < (HUB_SIZE-1); an++) {
			...
			if(bonded_neigh != P_INVALID) {
				_particle_particle_bonded_interaction<number, number4>(pos_hub, poss[bonded_neigh], F, box, true);
				if(MD_starr_model[0]) {
					bs_hub.n3 = bonded_neigh;
					_three_body<number, number4>(pos_hub, bs_hub, F, poss, n3_forces, n5_forces, box);
				}
			}
		}
	}
...
template<typename number, typename number4>
void CUDAStarrInteraction<number, number4>::compute_forces(CUDABaseList<number, number4> *lists, number4 *d_poss, GPU_quat<number> *d_orientations, number4 *d_forces, number4 *d_torques, LR_bonds *d_bonds, CUDABox<number, number4> *d_box) {
	...
	if(this->_mode != StarrInteraction<number>::STRANDS && this->_starr_model) {
	...
	if(_v_lists != NULL) {
		if(_v_lists->use_edge()) throw oxDNAException("use_edge unsupported by StarrInteraction");
			...
			Starr_forces<number, number4>
				<<<this->_launch_cfg.blocks, this->_launch_cfg.threads_per_block>>>
				(d_poss, d_forces, _v_lists->_d_matrix_neighs, _v_lists->_d_number_neighs, d_bonds, _d_strand_ids, d_box);
