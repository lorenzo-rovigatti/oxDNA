...
__device__ void _patchy_patchy_interaction(number4 &ppos, number4 &qpos, number4 &p_axis, number4 &q_axis, number4 &F, number4 &torque, CUDABox<number, number4> *box) {
...
template <typename number, typename number4>
__device__ void _patchy_star_interaction(number4 &ppos, number4 &qpos, number4 &F, CUDABox<number, number4> *box) {
	number4 r = box->minimum_image(ppos, qpos);
	number sqr_r = CUDA_DOT(r, r);
...
template <typename number, typename number4>
__device__ void _star_star_interaction(number4 &ppos, number4 &qpos, number4 &F, CUDABox<number, number4> *box) {
	number4 r = box->minimum_image(ppos, qpos);
	number sqr_r = CUDA_DOT(r, r);
...
template <typename number, typename number4>
__global__ void NS_forces(number4 *poss, GPU_quat<number> *orientations, number4 *forces, number4 *torques, CUDABox<number, number4> *box) {
	...
	for(int j = 0; j < MD_N[0]; j++) {
		if(j != IND) {
			...
			if(type == NathanStarInteraction<number>::PATCHY_PATCHY) {
				...
				_patchy_patchy_interaction<number, number4>(ppos, qpos, a3, b3, F, T, box);
			}
			else if(type == NathanStarInteraction<number>::PATCHY_POLYMER) _patchy_star_interaction<number, number4>(ppos, qpos, F, box);
			else _star_star_interaction<number, number4>(ppos, qpos, F, box);
		}
	}
...
template <typename number, typename number4>
__global__ void NS_forces(number4 *poss, GPU_quat<number> *orientations, number4 *forces, number4 *torques, int *matrix_neighs, int *number_neighs, CUDABox<number, number4> *box) {
	if(IND >= MD_N[0]) return;
...
template<typename number, typename number4>
void CUDANathanStarInteraction<number, number4>::compute_forces(CUDABaseList<number, number4> *lists, number4 *d_poss, GPU_quat<number> *d_orientations, number4 *d_forces, number4 *d_torques, LR_bonds *d_bonds, CUDABox<number, number4> *d_box) {
