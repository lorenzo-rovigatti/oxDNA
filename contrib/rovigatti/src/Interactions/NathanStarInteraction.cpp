...
template<typename number>
void NathanStarInteraction<number>::generate_random_configuration(BaseParticle<number> **particles, int N) {
	if(!_make_crystal) {
		BaseInteraction<number, NathanStarInteraction<number> >::generate_random_configuration(particles, N);
		return;
	}
