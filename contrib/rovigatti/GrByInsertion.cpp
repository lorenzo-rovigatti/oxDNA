...
std::string GrByInsertion<number>::get_output_string(llint curr_step) {
	...
	number ref_energy = this->_config_info.interaction->get_system_energy(this->_config_info.particles, N);
	...
	for(int i = 0; i < _n_bins; i++) {
		...
		for(int j = 0; j < _insertions; j++) {
			...
			number energy = _TSP_inter->get_system_energy(this->_config_info.particles, N);
			// arbitrary threshold
			if(energy < 10000.) {
