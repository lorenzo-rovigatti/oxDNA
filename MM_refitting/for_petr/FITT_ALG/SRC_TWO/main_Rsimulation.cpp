...
//	cout << "AND THE SIZE IS " << sizeof(STATE_struct) << endl;
 ...
 if (argc != 7)
 {
   cout << "Usage " << argv[0] << " log_file_prefix " <<  " parameter_init_file.txt " << " [H/ST/HST] " << " iteration_no " << " beta_init " << " (file with sequence_file simulation_data_file ....)"   << endl;
   return 0;
 }
...
// sim.simulated_annealing_with_widths(total_iters,beta_init);
 ...
 sim.simulated_annealing(total_iters,beta_init);

 sim.show_all_melts(cout);
//cout << sim;
//sim.states[0].calculate_new_states(sim.str_act);
