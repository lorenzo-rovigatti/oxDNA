...
int Anneal_Simulation::load_sequences(const char *seqnamefile, double beta_0, double beta_start, double beta_step, int beta_array_length, PAR_struct &original_parameters, double box, string& simtype)
{
 ...
 if(!fin.good())
 {
	 throw string(string("Cannot open ") + string(seqnamefile));
 }
 int lines = 0;
 char readline[4096];
 string sequence;
...
Anneal_Simulation::Anneal_Simulation(vector<string> &sequence_files, vector<string> &data_files, string& parameter_file, const char *simtype, double _sim_beta,  double pmax, const char * logfile_prefix,  ostream& log ) 
 ...
 // states[0].check_no_of_bonded_states();
  ...
  for(int i = 0; i < total_sequences; i++)
  {
    ...
    if(current_melts[i] <= 0)
    {
      throw string( string("Error while calculating temperature for ") + string(sequences[i]) +  string(", try changing interpolation intervals"));
...
void Anneal_Simulation::set_sim_type(string simtype)
{
	   ...
	   if( simtype.find(string("WC")) != string::npos)
	   {
	 	  wc_only = true;
	 	  changed = true;
	 	  log << " Allowing Watson-Crick base pairs only " << endl;
	   }
...
//-------------------------------------------------------------------------------------
int Anneal_Simulation::calculate_attempt_melts(PAR_struct &pars, int myid, int total_id)
{
	int states_to_process = states.size() / total_id;
	int stop = (myid+1)*states_to_process;
	if(myid == total_id - 1)
...
//-------------------------------------------------------------------------------------
 ...
 {
   int accepted = 0;
   str_new.perturb_parameter(perturb_max,average_only,wc_only);
  ...
  // cout << "Parspert " << str_new << endl;
   ...
   if( !calculate_attempt_melts(str_new) ) //automatic rejection
   {
     str_new = str_act;
     return 0;
   }
...
void Anneal_Simulation::show_all_melts(ostream& out, double cutoff)
 {
   ...
   for(int i =0; i < states.size(); i++)
   {
     ...
     if(cutoff <= 0 || fabs(diff) > cutoff) 
        out << sequences[i] << " "  << compsequences[i] << " " <<   diff << " " << beta_to_kelvin(current_melts[i]) << " " << beta_to_kelvin(correct_melts[i]) << endl;
...
int Anneal_Simulation::MPI_simulation_MC_step(double sim_beta)
{
  ...
  if(id == 0)
  {
	   ...
	   {
		if(attempt_melts[i] <= 0) //this is an automatic rejection,reach uncalculable Tm
		{
		  str_new = str_act;
	          accepted = 0;
		  return accepted;
