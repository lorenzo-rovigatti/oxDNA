...
inline number MD_CPUBackend<number>::_particle_particle_bonded_interaction(Particle<number> *p) {
	...
	if(p->n3 != P_VIRTUAL) {
		...
		// here we define rbackghost, which is the position the backbone would be in if we were using the no-grooving model. We do this because some of the force calculations rely on this definition of rback; it's just easier than recalculating the constants
		// we use that in the no-grooving model: rback = a1*gamma + rstack - b1*gamma
		// NB if major-minor grooving is not in use, rback = rbackghost and everything works as it should
		//LR_vector<number> rbackghost = r + q->pos_stack - p->pos_stack - b1*GAMMA + a1*GAMMA;
		//LR_vector<number> rbackghost = r + q->orientation*LR_vector<number>(1,0,0)*POS_BACK - p->orientation*LR_vector<number>(1,0,0)*POS_BACK;
		//LR_vector<number> rbackghost = rback; //THIS ONE GIVE IDENTICAL NUMBERS
		...
		*/
		number dcosphi1dr    = (SQR(rstackmod)*ra2 - ra2*SQR(rbackghostmod) - rstackmod*(a2b1 + ra2*(-ra1 + rb1))*gamma + a2b1*(-ra1 + rb1)*SQR(gamma))/ rbackghostmodcub;
		number dcosphi1dra1  =  rstackmod * gamma * parentesi / rbackghostmodcub;
		number dcosphi1dra2  = -rstackmod / rbackghostmod;
		number dcosphi1drb1  = -rstackmod * gamma * parentesi / rbackghostmodcub;
		number dcosphi1da1b1 = -SQR(gamma) * parentesi / rbackghostmodcub;
		number dcosphi1da2b1 =  gamma / rbackghostmod;
		...
		*/
		number dcosphi2dr    =  (parentesi * (rstackmod + (rb1 - ra1)*gamma) - ra2*SQR(rbackghostmod)) / rbackghostmodcub;
		number dcosphi2dra1  = -rstackmod*gamma*(rstackmod*ra2 + a2b1*gamma) / rbackghostmodcub;
		number dcosphi2dra2  = -rstackmod / rbackghostmod;
		number dcosphi2drb1  =  rstackmod*gamma* parentesi / rbackghostmodcub;
		number dcosphi2da1b1 = -SQR(gamma)* parentesi / rbackghostmodcub;
		number dcosphi2da2b1 = -gamma / rbackghostmod;
...
;
	...
	if(CXST_RCLOW < rstackmod && rstackmod < CXST_RCHIGH) {
		...
		*/
		...
		//LR_vector<number> rbackboneghost = r + q->pos_stack - p->pos_stack - b1 * GAMMA + a1 * GAMMA;
		//LR_vector<number> rbackboneghost = r + q->pos_back - p->pos_back; //THIS ONE GIVES IDENTICAL NUMBERS
		LR_vector<number> rbackboneghost = r + b1 * POS_BACK - a1 * POS_BACK;
		//LR_vector<number> rbackboneghost = r + q->orientation*LR_vector<number>(1,0,0)*POS_BACK - p->orientation*LR_vector<number>(1,0,0)*POS_BACK;
		...
		if(cxst_energy != 0.) {
			...
			*/
			number dcdr    = -gamma * parentesi * (gamma * (ra1 - rb1) + rstackmod) / rbackghostmodcub;
