...
//---------------------------------------------------------------------------------------
template <typename number> void Serialized_particle_force_torque<number>::read_from(Particle<number> &par)
{
	index = par.index;
	torque = par.torque;
	force = par.force;
...
//----------------------------------------------------------------------------------------
template <typename number> int Serialized_particle_force_torque<number>::add_to(Particle<number> &par)
{
	if(par.index != index)
	{
...
//---------------------------------------------------------------------------------------
template <typename number> void Serialized_particle_position<number>::read_from(Particle<number> &par)
{
	index = par.index;
	pos = par.pos;
...
//----------------------------------------------------------------------------------------
template <typename number> int Serialized_particle_position<number>::write_to(Particle<number> &par)
{
	if(par.index != index)
	{
...
int MD_MPIBackend<number>::_MPI_send_block_data(void *data,size_t size,int node_to,int TAG)
{
	  ...
	  if(ret != MPI_SUCCESS)
	  {
		   	this->_IO->die("Error while sending MPI message");
	  }
	  else return 1;
...
int MD_MPIBackend<number>::_MPI_receive_block_data(void *data, size_t size, int node_from, int TAG)
{
	     ...
	     if(ret != MPI_SUCCESS)
	     {
	    	this->_IO->die("Error while receving MPI message");
...
//---------------------------------------------------------------------------------------
template<typename number>
MD_MPIBackend<number>::MD_MPIBackend(IOManager *IO) : MD_CPUBackend<number>(IO) {
...
template<typename number> int MD_MPIBackend<number>::_MPI_receive_serialized_particle(int from_id)
{
	...
	if(part.index < 0 || part.index >= this->_N)
		this->_IO->die(" Received particle with impossible index %d",part.index);
...
template<typename number> void MD_MPIBackend<number>::_Evaluate_my_particles(void)
{
	...
	for(int i = this->_info_process.min_index; i <= this->_info_process.max_index; i++) {
		...
		while(neigh != P_VIRTUAL) {
			this->_U += MD_CPUBackend<number>::_particle_particle_interaction(p, &this->_particles[neigh]);
			neigh = p->next_neighbour();
		}
	}
...
template<typename number> int MD_MPIBackend<number>::_MPI_send_all_serialized_particles_with_neighbors_to_slave(int slave_id)
{
  ...
  {
	  this->_IO->die("Trying to send data to nonexistent node %d",slave_id);
		...
		for(int i = 0; i < this->_N; i++)
		{
		  ...
		  if(interactions_per_node_filled >= this->_interaction_per_node || i == this->_N-1)
		  {
			...
			if(process_to_send_data >= this->_proc_size) //this is just for evaluation by the master node 0
			{
				...
				{
					this->_IO->die(" Error in MPI data distribution, bug in communication master->slave enumeration of states, filled %d out of %d ",interactions_filled,this->_total_no_of_interactions );
				}
...
//---------------------------------------------------------------------
template<typename number>
void MD_MPIBackend<number>::print_energy(llint curr_step) {
	if(this->_myid == 0)
	{
		//cout << "This is process " << this->_myid << " and I am printing energy " << endl;
		this->_IO->print_energy(*this, curr_step);
	}
}
//---------------------------------------------------------------------------
