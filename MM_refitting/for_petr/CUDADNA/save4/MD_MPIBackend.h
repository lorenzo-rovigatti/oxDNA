...
template<typename number> struct Serialized_particle_position
{
	...
	int _N_neigh;
	void read_from(Particle<number> &par);
	int write_to(Particle<number> &par);
};
...
template<typename number> struct Serialized_particle_force_torque
{
	...
	void read_from(Particle<number> &par);
	int add_to(Particle<number> &par);
...
protected:
   ...
   // int *_communicate_particles; //communicate_particles is a list of particles to process, which is sent by master node
	...
	//number _MPI_particle_particle_bonded_interaction(Particle<number> *p);
	//number _MPI_particle_particle_interaction(Particle<number> *p, Particle<number> *q);
