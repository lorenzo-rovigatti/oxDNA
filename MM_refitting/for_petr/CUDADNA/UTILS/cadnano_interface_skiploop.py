...
DEBUG = 0
DIST=2.55 #distance between centres of virtual helices
...
def print_usage():
    # cadnano_type: either sq or he
    # nupack: produce a scaffold-staple pattern file for feeding to nupack
    # side: choose the size of the simulation box
    print "usage: %s cadnano_type [nupack] [side]" % sys.argv[0]
    sys.exit()
...
def insert_loop_skip(strands, start_pos, dir, perp, rot, helix_angles, vhelix, nodes, use_seq, seqs):
    ...
    for i in range(len(nodes.begin)):
        ...
        if end_gs - begin_gs != 0:
            ...
            #except:
             #   print "debug:"
              #  print vhelix.num, len(helix_angles_new), begin_gs, end_gs, len(new_angle), i
               # sys.exit()
...
def add_slice_nupack(vhelix, strand_number, begin_helix, end_helix, index_lookup, strand_type):
    if (strand_type + vhelix.num % 2) % 2 == 0:
        for nucleotide in range(end_helix - begin_helix + 1):
            vhelix_base = nucleotide + begin_helix
            if strand_type == 0:
                index_lookup[(vhelix.num,vhelix_base)] = [strand_number, nucleotide]
            elif strand_type == 1:
                index_lookup[(strand_number,nucleotide)] = [vhelix.num, vhelix_base]
            else:
                base.Logger.log("add_slice_nupack(): unknown strand type, dying", base.Logger.CRITICAL)
                sys.exit()

    else:
        for nucleotide in range(begin_helix + 1 - end_helix):
            vhelix_base = begin_helix - nucleotide
            if strand_type == 0:
                index_lookup[(vhelix.num,vhelix_base)] = [strand_number, nucleotide]
            elif strand_type == 1:
                index_lookup[(strand_number,nucleotide)] = [vhelix.num, vhelix_base]
...
def main():
    ...
    if len(sys.argv) > 2:
        if sys.argv[2] == "nupack":
            ...
            # read 3rd arg as system size in oxDNA simulation units
    ...
    base.Logger.log("using json file dummy.json", base.Logger.INFO)
    ...
    if origami_sq:
        ...
        base.Logger.log("something went wrong", base.Logger.CRITICAL)
        sys.exit()
    ...
    if nupack == 0:
        ...
        if DEBUG:
            print "=================="
            print vhelix_to_scaffold_final
            print "==================="
            print staple_to_vhelix_final
            counter = 0
            counter_threshold = 0
            for line in staple_to_scaffold:
                print line
                counter_threshold += len(line)
            for line1 in staple_to_scaffold:
                for i in line1:
                    for line2 in staple_to_scaffold:
                        for j in line2:
                            if i == j:
                                counter += 1
            if counter != counter_threshold:
                print "ERROR: either repeated scaffold base found in staple_to_scaffold, or bases missing"

    if nupack:
        print "number of strands: " + str(len(staple_to_scaffold)+1)
    if not nupack:
        print "number of strands: " + str(len(final_sys._strands))
        print "box size: " + str(side)
...
def square_type_test():
    cadsys = parse_cadnano ('dummy.json')
    for vhelix in cadsys.vhelices:
        print 'vhelix ' + str(vhelix.num)
        print 'scaffold'
        i = 0
        for s in vhelix.scaf:
            print str(i) + '  ' + s.type(vhelix)
            i += 1
        print 'staple'
        i = 0
        for s in vhelix.stap:
            print str(i) + '  ' + s.type(vhelix)
            i += 1
...
def nodes_test():
    ...
    for vhelix in cadsys.vhelices:
        print vhelix.num
        nodes = build_nodes(vhelix)
        for i in range(len(nodes.begin)):
            print str(i) + '  ' + str(nodes.begin[i]) + ' ' + str(nodes.end[i])
