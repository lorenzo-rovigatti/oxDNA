...
template<typename number> PT_VMMC_CPUBackend<number>::PT_VMMC_CPUBackend(IOManager *IO) : VMMC_CPUBackend<number> (IO) {
	// parallel tempering
	_npttemps = 4;
	_pttemps = NULL;
	_my_mpi_id = -1;
...
void PT_VMMC_CPUBackend<number>::init (char conf_filename[256]) {
	...
	if (this->_my_mpi_id == 0) {
		...
		if (!check2) {
			...
			//this->_IO->log(this->_IO->LOG_INFO, "Attention: temperatures are not in increasing order. Hoping for the best");
		}
	}
	
	// let's get our own temperature...
	if (_npttemps != _mpi_nprocs) this->_IO->die("Number of PT temperatures does not match number of processes (%d != %d)", _npttemps, _mpi_nprocs);
	this->_T = _pttemps[_my_mpi_id];
	//this->_IO->log(this->_IO->LOG_INFO, "Replica %d: Running at T=%g", _my_mpi_id, this->_T);
...
void PT_VMMC_CPUBackend<number>::get_settings(input_file & inp) {
	...
	if (getInputString(&inp, "pt_temp_list", tstring, 1) == KEY_FOUND) {
		this->_IO->log(this->_IO->LOG_INFO, "Running Parallel Tempering at temperatures .... %s\n", tstring);
		char * aux, deg;
		int c = 0, check;
		double * tmpt;
		tmpt = new double[100];
		...
		while (aux != NULL) {
			...
			if (check < 1) {
				this->_IO->die("Unrecognizable line in pt_temp_list");
			}
			if (check == 1) {
				; // do nothing
			}
			if (check == 2) {
				...
				case 'k':
					...
					break;
				default:
					this->_IO->die("Unrecognizable temperature '%s' in pt_temp_list", tmpt[c]);
					break;
				}
			}
			c++;
		...
		if (c == 0) {
			this->_IO->die("Nothing found in pt_temp_list");
		} else {
...
template <typename number>
void PT_serialized_particle_info<number>::read_from (Particle<number> &par) {
	pos = par.pos;
	orientation= par.orientation;
}

template <typename number>
void PT_serialized_particle_info<number>::write_to (Particle<number> &par) {
	par.pos = pos;
...
template<typename number>
void PT_VMMC_CPUBackend<number>::print_energy(llint curr_step) {
	VMMC_CPUBackend<number>::_compute_energy ();
