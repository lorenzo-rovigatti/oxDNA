...
template<typename number>
MutualTrap<number>::MutualTrap(number stiff, number r0, Particle<number> * p) : ExternalForce<number>(0, 0, LR_vector<number> (0, 0, 0)) {
    this->_stiff = stiff;
    this->_r0 = r0;
    this->_p_ptr = p;
}
...
LR_vector<number> MutualTrap<number>::value (llint step, LR_vector<number> &pos) {
    LR_vector<number> dr = this->_p_ptr->pos - pos; // other - self
    return (dr / dr.module()) * (dr.module() - _r0) * ((number) 0.5) * this->_stiff;
}

template<typename number>
number MutualTrap<number>::potential (llint step, LR_vector<number> &pos) {
    LR_vector<number> dr = this->_p_ptr->pos - pos;
...
template<typename number>
RepulsionPlaneMoving<number>::RepulsionPlaneMoving(number stiff, Particle<number> *p, LR_vector<number> dir) : ExternalForce<number>(0,0,dir) {
	this->_stiff = stiff;
...
LR_vector<number> RepulsionPlaneMoving<number>::value(llint step, LR_vector<number> &pos) {
	number distance = (pos - _p_ptr->pos) * this->_direction;
	if (distance >=  0)
