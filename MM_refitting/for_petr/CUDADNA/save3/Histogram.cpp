...
#include "Histogram.h"
#include "OrderParameters.h" // forward declaration in header
...
void Histogram::init (const char * filename, OrderParameters * op, double  * temps, int ntemps) {
	...
	fprintf (stderr, "Histogram parser: parsing `%s\'\n", filename);
	...
	fprintf (stderr, "Histogram found: %d, %d %d\n", _ndim, _dim, _ntemps);
	
	// read the file linewise...
	char line[512];
	int lineno = 1;
	inp.getline(line, 512); // to remove the first line
	while (inp.good()) {
		...
		//fprintf (stderr, "Histogram parser: parsing line: \"%s\"\n", aux);
		...
		if (check != _ndim + 2 + _ntemps) {
			fprintf (stderr, "Histogram parser: error parsing line %d in %s. Not enough numbers in line. Aborting\n", lineno, filename);
			abort ();
		}
		...
		for (int i = 0; i < _ndim; i ++) {
			if (tmp[i] < 0 || tmp[i] > (_sizes[i] + 2)) {
				fprintf (stderr, "Histogram parser: error parsing line %d of `%s\': index %d out of OrderParameters bounds. Aborting\n", lineno, filename, tmp[i]);
				abort ();
			}
			if (tmpf < -DBL_EPSILON) {
				fprintf (stderr, "Histogram parser: error parsing line %d of `%s\': weight %lf < 0. Cowardly refusing to proceed. Aborting\n", lineno, filename, tmpf);
				abort ();
			}
		}
	...
	fprintf (stderr, "Histogram parser: parsing done\n");
...
void Histogram::init (const char * filename, OrderParameters * op) {
	...
	//fprintf (stderr, "Histogram found: %d, %d\n", _ndim, _dim);
	
	// read the file linewise...
	char line[512];
	int lineno = 1;
	inp.getline(line, 512); // to remove the initial time info
	while (inp.good()) {
		inp.getline(line, 512);
		if (strlen (line) == 0)
			continue;
...
void Histogram::add (int i, double am, double w, double e_state, double e_stack) {
	double e0, et;
	// etot = e0 + T * et;
	et = e_stack * STCK_FACT_EPS / (STCK_BASE_EPS + _simtemp * STCK_FACT_EPS);
	e0 = e_state - _simtemp * et;
	...
	if (w > 0) {
		_rdata[i] += 1./w;
		for (int k = 0; k < _ntemps; k ++)
		  _erdata[k][i] += am * exp (- (e0 + _etemps[k] * et) / _etemps[k] + (e0 + _simtemp * et) / _simtemp) / w;
	}
	else {
		_rdata[i] += 1.;
		for (int k = 0; k < _ntemps; k ++)
		  _erdata[k][i] += am * exp (- (e0 + _etemps[k] * et) / _etemps[k] + (e0 + _simtemp * et) / _simtemp);
	}
}

void Histogram::add (int i, int am, double w, double e_state, double e_stack) {
	this->add (i, (double)am, w, e_state, e_stack);
}

void Histogram::print_to_file (const char * filename, long long int time, bool only_last) {
	FILE * outfile;
	...
	if (outfile == NULL) {
		fprintf (stderr, "Error: could not open histogram file `%s\' for writing. Aborting.\n", filename);
		abort ();
	}
