...
pars_from_modelh, vals_from_modelh = read_vanilla_parameters(model_file)    
model_file.close()

SD_par_file = open("oxDNA_sequence_dependent_parameters_in.txt",'r')   
over_indices, over_vals, stck_fact_eps_read, stck_fact_eps = read_pars_from_SD_file(SD_par_file)    
SD_par_file.close()
...
T = 0.1
OXPS_zero, shifts = init_oxpars(pars_from_modelh, vals_from_modelh, over_indices, over_vals,T,stck_fact_eps)
...
rclow, rchigh = find_cuts_for_lists(OXPS_zero)

tr_file = open("trajectory.dat",'r')
topo_file = open("generated.top", 'r')

fene_r, stck_r, th4_bn, th5, th6, cosphi1, cosphi2, types_bn, hydr_r, th1, th2, th3, th4_unbn, th7, th8, types_unbn = read_oxdna_trajectory_dist_and_angles(rclow, rchigh, tr_file, topo_file)
...
SYMM_INDEX1 = torch.tensor(SYMM_LIST,device=device)
SYMM_INDEX2 = torch.tensor(SYMM_LIST_SYMM,device=device)
...
#LIST OF INICES WITH PARAMETERS WHICH NEED CONTINUITY
...
def COST(PARS_OPTI) :
    ...
    CURR_PARS[SYMM_INDEX1] = CURR_PARS[SYMM_INDEX2]
    ...
    WEIGHTS = torch.exp( 10*torch.sum( EN_FENE_REW + EN_STCK_IN*(1-STCK_MOD_REW/STCK_MOD_IN) + EN_HYDR_IN*(1-HYDR_MOD_REW/HYDR_MOD_IN) + EN_CRST_33_IN*(1-CRST_33_MOD_REW/CRST_33_MOD_IN) +\
                                     EN_CRST_55_IN*(1-CRST_55_MOD_REW/CRST_55_MOD_IN),dim=1 ))
