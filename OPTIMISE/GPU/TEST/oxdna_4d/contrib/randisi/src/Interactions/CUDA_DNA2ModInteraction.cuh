...
struct particle{number4 pos,x,y,z;};
...
__device__ void _bonded_part(number4 &n5pos, number4 &n5x, number4 &n5y, number4 &n5z, number4 &n3pos,
	...
	// Because they are passed by reference, we probably need to define something else.
	
	/*
	//do something to tell what the angle should be
	number roll_angle = 0, tilt_angle = 0;
	if (q_in_group_1){ 
		roll_angle = mod_stacking_roll_1;
	else if (q_in_group_2){ 
		roll_angle = mod_stacking_roll_2;
	}
	*/
	//rotate the vectors around the base vector
  number4 n3x_1 = n3x;
...
//	*/
	...
	number t4 = CUDA_LRACOS(CUDA_DOT(n3z_1, n5z));
	number t5 = CUDA_LRACOS(CUDA_DOT(n5z, rstackdir));
	number t6 = CUDA_LRACOS(-CUDA_DOT(n3z_1, rstackdir));
	number cosphi1 = CUDA_DOT(n5y, rbackref) / rbackrefmod;
	number cosphi2 = CUDA_DOT(n3y_1, rbackref) / rbackrefmod;
	...
	if(energy != (number) 0) {
		...
		// THETA 6
		Ftmp += (n3z_1 + cosf(t6) * rstackdir) * (energy * f4t6Dsin / (f4t6 * rstackmod));
		...
		number rb1 = CUDA_DOT(rstackdir, n3x_1);
		number a2b1 = CUDA_DOT(n5y, n3x_1);
		...
		Ftmp -= (rstackdir * dcosphi1dr +
				...
				(n3x_1 - rb1*rstackdir) * dcosphi1drb1) / rstackmod) * force_part_phi1;

		// COS PHI 2
		// here particle p -> b, particle q -> a
		ra2 = CUDA_DOT(rstackdir, n3y_1);
		ra1 = rb1;
		rb1 = CUDA_DOT(rstackdir, n5x);
		a2b1 = CUDA_DOT(n3y_1, n5x);
		...
		Ftmp -= (rstackdir * dcosphi2dr +
			    ((n3y_1 - rstackdir * ra2) * dcosphi2dra2 +
				(n3x_1 - rstackdir * ra1) * dcosphi2dra1 +
		...
		// THETA 4
		Ttmp += _cross<number, number4>(n3z_1, n5z) * (-energy * f4t4Dsin / f4t4);
		...
		if(qIsN3) {
			...
			Ttmp += force_part_phi1 * dcosphi1drb1 * _cross<number, number4>(rstackdir, n3x_1);

			Ttmp += force_part_phi2 * dcosphi2dra2 * _cross<number, number4>(rstackdir, n3y_1) +
				force_part_phi2 * dcosphi2dra1 * _cross<number, number4>(rstackdir, n3x_1);
		}

		Ttmp += force_part_phi1 * dcosphi1da2b1 * _cross<number, number4>(n5y, n3x_1)
			+ _cross<number, number4>(n5x, n3x_1) * force_part_phi1 * dcosphi1da1b1;

		Ttmp += force_part_phi2 * dcosphi2da2b1 * _cross<number, number4>(n5x, n3y_1) +
			_cross<number, number4>(n5x, n3x_1) * force_part_phi2 * dcosphi2da1b1;
...
__global__ void dna_forces_edge_bonded(number4 *poss, GPU_quat<number> *orientations,  number4 *forces, number4 *torques, LR_bonds *bonds, bool grooving, bool use_oxDNA2_FENE, bool use_mbf, number mbf_xmax, number mbf_finf, number *_d_stacking_roll, number *_d_stacking_r_roll, number *_d_stacking_tilt, number *_d_stacking_multiplier) {
	...
	if(bs.n5 != P_INVALID) {
		...
		number4 b1,b2,b3;
		get_vectors_from_quat<number,number4>(orientations[bs.n5], b1, b2, b3);
		struct particle<number4> q={qpos,b1,b2,b3};
		//TODO: assuming that IND is actually the index of the particle - otherwise this will do strange things
		int n3_index = IND;
		number stacking_roll = _d_stacking_roll[n3_index];
		//number stacking_r_roll = _d_stacking_r_roll[n3_index];
...
template<typename number, typename number4>
__forceinline__ __device__ number4 CUDA_rotateVectorAroundVersor(number4& vector, number4& versor, number& angle){
	...
	number4 to_return;
	to_return.x = versor.x * scalar * (1. - costh) + vector.x*costh + cross.x * sinth;
	to_return.y = versor.y * scalar * (1. - costh) + vector.y*costh + cross.y * sinth;
	to_return.z = versor.z * scalar * (1. - costh) + vector.z*costh + cross.z * sinth;
	to_return.w = 0.;
	return to_return;  
}
