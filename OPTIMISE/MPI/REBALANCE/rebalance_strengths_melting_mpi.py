...
if len(sys.argv) != 3 :
    ...
    print("Usage: python3 optimise.py config_file first_step_flag")
    sys.exit()
...
if cg.rank_seq == 0:
    ...
    print("reweighting weights")
    print("saving to "+cg.fin_wfiles[cg.seq_id])
    min_d = 10000
    cid = 0
    
    for t in range(len(cg.rew_Ts[cg.seq_id])) :
        if abs(cg.current_mT-cg.rew_Ts[cg.seq_id][t]) < min_d :
            min_d = abs(cg.current_mT-cg.rew_Ts[cg.seq_id][t])
            cid = t
        
    functions_melting.estimate_new_wfile(cg.current_hist_Ts[cid],cg.fin_wfiles[cg.seq_id])
