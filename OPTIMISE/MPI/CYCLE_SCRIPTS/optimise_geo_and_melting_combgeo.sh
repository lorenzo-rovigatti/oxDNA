...
if [ "$#" -ne 3 ]; then
	...
	echo "bash optim....sh config_melting config_geo nsteps"
	exit 1
fi

config_melting=$1 #configuration file melting
config_geo=$2	#number of reps for geometry
...
st=$(awk '$1 == "SEQ" {print $2}' $path_geo/$config_geo)
if [[ "$st" == "" ]]; then
...
echo "Sequences geometry:"

for (( k = 0; k < ${Nseq_geo}; k++ )); do
	echo ${Seq_geo[$k]}
done
...
Nreps_geo=$(awk '$1 == "REPS" {print $2}' $path_geo/$config_geo)
if [[ "$Nreps_geo" == "" ]]; then
...
for ((l=0; l < ${Nseq_geo}; l++)); do
	for ((j=0; j < ${Nreps_geo};j++)); do
		...
		sed -i "s|replace_me2|${par_path}|g" input2.an
...
#preparing par files
cp ${path_melting}/oxDNA_sequence_dependent_parameters_tmp.txt ${path_geo}/oxDNA_sequence_dependent_parameters_in.txt


#fxed parameters

cd ${path_geo}
python3 ${opti_path}/Utils/prepare_fixed_pars_file_from_config.py ${path_geo}/${config_geo} oxDNA_sequence_dependent_parameters_in.txt

mpirun -np ${Nproc_geo} python3 ${opti_path}/MPI/optimise_geometry_mpi.py ${config_geo} > OutOpti &

wait

mv ${path_geo}/oxDNA_sequence_dependent_parameters_tmp.txt ${path_geo}/oxDNA_sequence_dependent_parameters_fin.txt
...
mv ${path_geo}/OutOpti ${main_path}/OPTI_FILES/OPTI_OUTPUTS/OutOpti_geo${i}
