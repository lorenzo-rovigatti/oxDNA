...
for ((l=0; l < ${Nseq}; l++)); do
	for ((j=0; j < ${Nreps};j++)); do
		...
		cp ${opti_path}/input_VMMC ${main_path}/Step0/Seq${l}/Rep${j}/
		...
		cp ${opti_path}/input1_melting.an ${main_path}/Step0/Seq${l}/Rep${j}/
		cp ${opti_path}/input2_melting.an ${main_path}/Step0/Seq${l}/Rep${j}/
		...
		sed -i "s|seed = 1|seed = ${RANDOM}|g" input_VMMC
		sed -i "s|steps = 1e7|steps = ${timesteps}|g" input_VMMC
		sed -i "s|T = 72C|T = ${Ts[$l]}C|g" input_VMMC
		sed -i "s|salt_concentration = 0.15|salt_concentration = ${salt[$l]}|g" input_VMMC
...
#optimise
cd ${main_path}/Step0
mpirun -np ${Nproc} python3 ${opti_path}/MPI/optimise_melting_mpi.py ../$config > OutOpti
