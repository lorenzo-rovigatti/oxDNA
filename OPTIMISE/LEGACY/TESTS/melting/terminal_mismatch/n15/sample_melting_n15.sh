...
#SBATCH --job-name=melting_samp_n15
...
mTs=( 42 47 52 57 63 68 72 78 83 88 )
...
for ((l=0; l < ${Nseqs}; l++)); do
	for ((m=0; m < ${nreps}; m++)); do
		...
		sed -i "s|op_file = .*|op_file = ${OP_files[$l]}|g" input_VMMC
		sed -i "s|weights_file = .*|weights_file = ${W_files[$l]}|g" input_VMMC
