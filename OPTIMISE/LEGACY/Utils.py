...
def unscrumble_cov_diag(cov) :
    ...
    for i in range(cg.Nseq) :
        ...
        for j in range(len(cg.ids)) :
            ...
            for z in range(len(cov[i])) :
                if z%len(cg.ids) == j :
                    unscr_cov_single.append(cov[i][z][z])
                    
            unscrumbled_cov.append(unscr_cov_single)
            
        unscrumbled_cov_all_seqs.append(unscrumbled_cov)
        
    return unscrumbled_cov_all_seqs


def plot_cov_sampled_diag() :
    
    unscr_cov_sampled = unscrumble_cov_diag(cg.cov_sampled)
    unscr_cov_target = unscrumble_gs(cg.target_cov)
    
    for i in range(len(unscr_cov_sampled)) :    #NSeq
    
        for j in range(len(unscr_cov_sampled[i])) : #Nids
            ...
            x = []            
            for z in range(len(unscr_cov_sampled[i][j])) :
                x.append(z)
                
            ys = unscr_cov_sampled[i][j]
            yt = unscr_cov_target[i][j]
            ...
            ax.plot(x, ys, 'r', label="oxdna")
            ax.legend(fontsize = 20)
            plt.savefig("Seq"+str(i)+"_Cov_diag_"+coord_name+".pdf",bbox_inches='tight',pad_inches=0.05)  
