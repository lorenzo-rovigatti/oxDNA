    ...
    for line in cfile.readlines() :
        ...
        if(vals[0] == 'SEQ'):
            ...
            if len(vals) != 9 :
                ...
                print("Usage:")
                print("SEQ seq box_size Cs simT Nts DT opfile wfile")
                print("box_size = box_size")
                print("Cs = ssalt concentration")
                print("simT = simulation temperature")
...
def reweight_melting_temperature(par):
    
    if  cg.Diter_Trange > 0:
        update_rew_Ts()    
    
    mTs = [] #list of reweighted melting temperatures
    mTs_w = [] #list of reweighted melting temperatures - widths
        ...
        for m in range(cg.n_Ts[l]) :
            ...
            for n in range(cg.Nreps) :
                ...
                with oxpy.Context():              
                    ...
                    inp[nrep].init_from_filename("./Seq"+str(l)+"/Rep"+str(n)+"/input2_melting.an")
                    #create analysys backend
                    backend.append(oxpy.analysis.AnalysisBackend(inp[n]))
            ...
            for i in range(len(energy1)) :
                ...
                if energy1[i] != 999 and cg.energy_sampled[l][i] != 999:
                    ...
                    #if m == 5:
                    #    print(energy1[i],cg.energy_sampled[l][i],deltaH)
       
                    hist_Ts[m][cg.hbs_sampled[l][i]] += math.exp(-deltaH)/cg.weights[l][cg.hbs_sampled[l][i]]   #XXXtocheck do we have to compute hbs again?
                    #hist_Ts[m][cg.hbs_sampled[l][i]] += 1./cg.weights[l][cg.hbs_sampled[l][i]]   #XXXtocheck do we have to compute hbs again?   
        
        
        #compute reweighted melting temperature
        
            #print("T: " + str(cg.rew_Ts[l][m]))
            #print(hist_Ts[m])
...
def Cost_function_mT(par,par0):
    ...
    mTs, mTs_w = reweight_melting_temperature(par)
    ...
    for l in range(len(mTs)):
        ...
        #C += (mTs[l]-cg.target_mTs[l])*(mTs[l]-cg.target_mTs[l])/mTs_err[l]/mTs_err[l]
        C += (mTs[l]-cg.target_mTs[l])*(mTs[l]-cg.target_mTs[l])
