...
Njuns = 39 #total number of junctions
...
def Stiff(cov,m) :
    ...
    cov = cov *(math.pi/180/0.34)*(math.pi/180/0.34)
...
ax.tick_params(axis='both', which='major', labelsize=12)
ax.tick_params(axis='both', which='minor', labelsize=8)
ax.plot(q_rescaled_fit, fit_f(q_rescaled_fit*N/math.pi, X_At_ox2[0], X_At_ox2[1], X_At_ox2[2], X_At_ox2[3]), 'k-', label="Enrico's")
ax.scatter(q_rescaled,y0,label="At")
ax.plot(q_rescaled_fit, fit_f(q_rescaled_fit*N/math.pi, X_At[0], X_At[1], X_At[2], X_At[3]), 'b--', label=None)
ax.scatter(q_rescaled,y1,label="Ar")
ax.plot(q_rescaled_fit, fit_f(q_rescaled_fit*N/math.pi, X_Ar[0], X_Ar[1], X_Ar[2], X_Ar[3]), 'b--', label=None)
...
ax.plot(ms,lbs,label="$l_b$")
ax.plot(ms,lts_half,label="$l_t/2$")
ax.legend(fontsize = 20)
plt.savefig("persistence_lengths.pdf",bbox_inches='tight',pad_inches=0.05)
