...
class base :
    def __init__(self,oxc,ty):
        ...
        y = copy.deepcopy(oxc.base_v)
        z = -copy.deepcopy(oxc.normal)
        x = -copy.deepcopy(oxc.base_norv)
...
class base_pair :
    def __init__(self,b1,b2) :
        ...
        for i in range(3) :
            n1[i] = b1.frame.orientation[i,2]
            n2[i] = b2.frame.orientation[i,2]
        
        e3 = (n1-n2)*(1./np.linalg.norm(n1-n2))
        #print(e3)
        
        y = b1.frame.pos-b2.frame.pos
        y = y *(1./np.linalg.norm(y))
        #print(y)
        
        e2 = y-np.dot(e3,y)*e3
        
        e2 = e2*(1./np.linalg.norm(e2))
        #print(e2)
        
        e1 = np.cross(e2,e3)
        
        ori = np.column_stack((e1,e2,e3))
...
class junction :
    def __init__(self,bp1,bp2) :
        ...
        theta = math.acos(0.5*(np.trace(A2)-1))
        rot = np.zeros(3,dtype=float)
        rot[0] = theta/(2*math.sin(theta))*(A2[2,1]-A2[1,2])*180./math.pi
        rot[1] = theta/(2*math.sin(theta))*(A2[0,2]-A2[2,0])*180./math.pi
        rot[2] = theta/(2*math.sin(theta))*(A2[1,0]-A2[0,1])*180./math.pi
