...
seqs=("ATAATTTATATATCT" "ATTGTCATATATATC" "AATTAATTAAAGGGT" "AAAAACAGAAGTCAA" "TCAATTGGATGCGAC" "GGCACTGGGATCACT" "GCTGACTCTGGGGGG" "GGCCGCACCCAAGCC" "TGCGTCCGGCGCGCA" "CGCGCGCCGCGGGCA" )
OP_files=( "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" "op_n15.txt" )
W_files=( "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt" "wfile_n15.txt"  )
Nseqs=${#seqs[@]}
nbps=( 15 15 15 15 15 15 15 15 15 15 )
mTs=( 48 53 59 64 69 74 79 84 90 94 )
...
for ((l=0; l < ${Nseqs}; l++)); do
	for ((m=0; m < ${nreps}; m++)); do
        ...
        sub_script=PT_VMMC_seq${l}_rep${m}_n15.sh
        cp ${path}/PT_VMMC_seq0_rep0_n15.sh ${rep_path}/${sub_script}
		...
		bash ../generate_dangle.sh ${box_size}
        for ((k=0; k < ${NTs}; k++)); do
        ...
        sed -i "s|job-name=.*|job-name=PTVMMC_s${l}r${m}_n15|g" ${sub_script}
		...
		for ((n=-${in_n}; n<=${fin_n}; n++)); do
            ...
            #echo "delta = ${delta}"
            cp ${path}/Weights_n15/wfile_n15_${delta}.txt ${rep_path}/wfile_n15.txt${counts}
