...
mTs=( 18 26 12 25 29 34 41 41 37 40 )
...
for ((l=0; l < ${Nseqs}; l++)); do
	for ((m=0; m < ${nreps}; m++)); do
        ...
        sub_script=PT_VMMC_seq${l}_rep${m}_n5.sh
        cp ${path}/PT_VMMC_seq0_rep0_n5.sh ${rep_path}/${sub_script}
        ...
        sed -i "s|job-name=.*|job-name=PTVMMC_s${l}r${m}_n5|g" ${sub_script}
		...
		for ((n=-${in_n}; n<=${fin_n}; n++)); do
            ...
            #echo "delta = ${delta}"
            cp ${path}/Weights_n5/wfile_n5_${delta}.txt ${rep_path}/wfile_n5.txt${counts}
            if [ ${ex_T} = 0 ]; then
                ex_T=$(( ${ex_T}+1 ))
            fi
			if [ $n -lt ${fin_n} ]; then
