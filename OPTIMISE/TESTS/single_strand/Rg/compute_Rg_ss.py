...
from read_trajectory_ss import read_oxdna_trajectory_standard_order #Maddocks style (see theses)
...
for i in range(len(trajectory)) :
    ...
    for j in range(size) :
    	r = np.linalg.norm(trajectory[i][j].base1.pos)
        ave_r += r/size
        ave_r2 += r*r/size
    ave_r_v_t.append(ave_r)
    ave_r2_v_t.append(ave_r2)
    r = np.linalg.norm(trajectory[i][size-1].base1.pos - trajectory[i][0].base1.pos)
    ave_ete_v_t.apend(r)
