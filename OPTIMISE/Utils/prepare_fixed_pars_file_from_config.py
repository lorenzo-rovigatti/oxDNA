...
Given a par file and a conf file, this program loops through the par file and generates a fixed par file without
the parameters used for optimisation (specified in config file). Note that symmetries and continuity are accounted for. 
"""
...
if len(sys.argv) != 3 :
    ...
    print("Usage: python3 prepare_fixed_pars_file_from_config.py config_file par_file")
    sys.exit(1)
...
fixed_par_file = open("oxDNA_sequence_dependent_parameters.txt",'w')
