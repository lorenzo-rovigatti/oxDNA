...
DNA2withRNA2Interaction::DNA2withRNA2Interaction() :
				DNAwithRNAInteraction() {
	ADD_INTERACTION_TO_MAP(DEBYE_HUCKEL, _debye_huckel);
	//ADD_INTERACTION_TO_MAP(COAXIAL_STACKING_DNA, _coaxial_stacking_DNA);

	F2_K_DNA[1] = CXST_K_OXDNA2;

	F4_THETA_T0_DNA[10] = CXST_THETA1_T0_OXDNA2;

	F4_THETA_SA_DNA[10] = CXST_THETA1_SA;
	F4_THETA_SB_DNA[10] = CXST_THETA1_SB;

	_salt_concentration = 0.5;
	_debye_huckel_half_charged_ends = true;

	_grooving_DNA = true;
	_fene_r0_DNA = FENE_R0_OXDNA2;
}

number DNA2withRNA2Interaction::pair_interaction_bonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(compute_r) {
...
number DNA2withRNA2Interaction::pair_interaction_nonbonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(compute_r) {
...
void DNA2withRNA2Interaction::get_settings(input_file &inp) {
...
number DNA2withRNA2Interaction::_debye_huckel(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	...
	//Calculating LR_vector depending on nucleotide types
	LR_vector rback;
    ...
    else if(this->_is_DNA(p)) {
    	rback = _computed_r + q->int_centers[RNANucleotide::BACK] - p->int_centers[DNANucleotide::BACK];
    }

    else {
    	rback = _computed_r + q->int_centers[DNANucleotide::BACK] - p->int_centers[RNANucleotide::BACK];
    }
	...
	if(rbackmod < _debye_huckel_RC) {
		...
		if(update_forces) {
			...
			//Updating torques depending on nucleotide types
			if(_interaction_type(p,q) == 0) {
...
number DNA2withRNA2Interaction::_coaxial_stacking_DNA(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(p->is_bonded(q)) {
		return (number) 0.f;
	}
...
number DNA2withRNA2Interaction::_fakef4_cxst_t1_DNA(number t, void *par) {
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1.0001))
		throw oxDNAException("In function DNA2withRNA2Interaction::_fakef4() t was found to be out of the range [-1,1] by a large amount, t = %g", t);
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1))
		t = (number) copysign(1, t);
	return _f4_DNA(acos(t), *((int*) par)) + _f4_pure_harmonic_DNA(acos(t), *((int*) par));
}

number DNA2withRNA2Interaction::_fakef4D_cxst_t1_DNA(number t, void *par) {
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1.0001))
		throw oxDNAException("In function DNA2withRNA2Interaction::_fakef4() t was found to be out of the range [-1,1] by a large amount, t = %g", t);
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1))
		t = (number) copysign(1, t);
	return -_f4Dsin_DNA(acos(t), *((int*) par)) - _f4Dsin_pure_harmonic_DNA(acos(t), *((int*) par));
}

number DNA2withRNA2Interaction::_f4_pure_harmonic_DNA(number t, int type) {
	// for getting a f4t1 function with a continuous derivative that is less disruptive to the potential
	number val = (number) 0;
	t -= F4_THETA_SB_DNA[type];
	if(t < 0)
		val = (number) 0.f;
	else
		val = (number) F4_THETA_SA_DNA[type] * SQR(t);

	return val;
}

number DNA2withRNA2Interaction::_f4Dsin_pure_harmonic_DNA(number t, int type) {
	// for getting f4t1 function with a continuous derivative that is less disruptive to the potential
	number val = (number) 0;
	number tt0 = t - F4_THETA_SB_DNA[type];
	if(tt0 < 0)
		val = (number) 0.f;
	else {
		number sint = sin(t);
		if(SQR(sint) > 1e-8)
			val = (number) 2 * F4_THETA_SA_DNA[type] * tt0 / sint;
		else
			val = (number) 2 * F4_THETA_SA_DNA[type];
	}
...
number DNA2withRNA2Interaction::_f4D_pure_harmonic_DNA(number t, int type) {
	// for getting f4t1 function with a continuous derivative that is less disruptive to the potential
	number val = (number) 0;
	number tt0 = t - F4_THETA_SB_DNA[type];
	if(tt0 < 0) {
		...
		val = (number) 2 * F4_THETA_SA_DNA[type] * tt0;
	}
