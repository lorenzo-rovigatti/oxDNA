...
number PSPInteraction::_repulsive_lj(const LR_vector &r, LR_vector &force, number sigma, number rstar, number b, number rc, bool update_forces) {
...
number PSPInteraction::debug(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces){
	auto *pCG = dynamic_cast<CCGParticle*>(p);
...
number PSPInteraction::patchy_interaction(BaseParticle *p, BaseParticle *q, bool compute_r,bool update_forces){
...
double PSPInteraction::exc_vol_bonded(BaseParticle *p, BaseParticle *q, bool compute_r,bool update_forces){
	if(compute_r) {
		_computed_r = _box->min_image(p->pos,q->pos);
...
double PSPInteraction::exc_vol_nonbonded(BaseParticle *p, BaseParticle *q, bool compute_r,bool update_forces){
...
bool PSPInteraction::color_compatibility(BaseParticle *p, BaseParticle *q){
	// auto *pCCG = static_cast<CCGParticle*>(p);
