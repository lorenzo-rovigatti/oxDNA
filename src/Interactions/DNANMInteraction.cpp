...
DNANMInteraction::DNANMInteraction() : DNA2Interaction() { // @suppress("Class members should be properly initialized")
	...
	this->_int_map[SPRING] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces)) &DNANMInteraction::_protein_spring;
	this->_int_map[PRO_EXC_VOL] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces)) &DNANMInteraction::_protein_exc_volume;

	//Protein-DNA Function Pointers
	this->_int_map[PRO_DNA_EXC_VOL] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces)) &DNANMInteraction::_protein_dna_exc_volume;

	//DNA Methods Function Pointers
	this->_int_map[this->BACKBONE] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces)) &DNANMInteraction::_dna_backbone;
	this->_int_map[this->COAXIAL_STACKING] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_coaxial_stacking;
	this->_int_map[this->CROSS_STACKING] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_cross_stacking;
	this->_int_map[this->BONDED_EXCLUDED_VOLUME] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_bonded_excluded_volume;
	this->_int_map[this->STACKING] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_stacking;
	this->_int_map[this->HYDROGEN_BONDING] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_hydrogen_bonding;
	this->_int_map[this->NONBONDED_EXCLUDED_VOLUME] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_nonbonded_excluded_volume;
	this->_int_map[this->DEBYE_HUCKEL] = (number (DNAInteraction::*)(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces))  &DNANMInteraction::_dna_debye_huckel;
    */
...
void DNANMInteraction::check_input_sanity(std::vector<BaseParticle *> &particles){
	DNA2Interaction::check_input_sanity(particles);
...
void DNANMInteraction::allocate_particles(std::vector<BaseParticle*> &particles) {
	...
	if (ndna==0 || ndnas==0) {
        ...
        for (int i = 0; i < npro; i++) particles[i] = new ACParticle();
    } else if (npro == 0) {
	    ...
	    if (_firststrand > 0){
            for (int i = 0; i < ndna; i++) particles[i] = new DNANucleotide(this->_grooving);
            for (int i = ndna; i < N; i++) particles[i] = new ACParticle();
	    } else {
            for (int i = 0; i < npro; i++) particles[i] = new ACParticle();
            for (int i = npro; i < N; i++) particles[i] = new DNANucleotide(this->_grooving);
	    }
	}
    ...
    while (topology.good()) {
        ...
        if (i == N_from_conf)
            throw oxDNAException(
                    "Too many particles found in the topology file (should be %d), aborting", N_from_conf);
...
//            }
    ...
    if (i < N_from_conf)
        throw oxDNAException(
                "Not enough particles found in the topology file (should be %d). Aborting", N_from_conf);
...
number DNANMInteraction::pair_interaction_bonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    ...
    if (p->btype >= 0 && q->btype >=0){
        ...
        number energy = _dna_backbone(p, q, false, update_forces);
        energy += _dna_bonded_excluded_volume(p, q, false, update_forces);
        energy += _dna_stacking(p, q, false, update_forces);
        return energy;
    }
...
number DNANMInteraction::pair_interaction_nonbonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    ...
    if (p->btype >= 0 && q->btype >= 0) { //DNA-DNA Interaction
        ...
        number energy = _dna_nonbonded_excluded_volume(p, q, false, update_forces);
        energy += _dna_hydrogen_bonding(p, q, false, update_forces);
        energy += _dna_cross_stacking(p, q, false, update_forces);
        energy += _dna_coaxial_stacking(p, q, false, update_forces);
        energy += _dna_debye_huckel(p, q, false, update_forces);
        return energy;
    }
...
//        }
        number energy = _protein_dna_exc_volume(p, q, &_computed_r, update_forces);
        return energy;
    }
...
number DNANMInteraction::_protein_dna_exc_volume(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces)
{
...
number DNANMInteraction::_protein_exc_volume(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces) {
    if (p->index != q->index && (p->btype < 0 && q-> btype < 0 )  ){
        LR_vector force(0,0,0);

        number energy =  DNANMInteraction::_protein_repulsive_lj(*r, force, update_forces);
...
number DNANMInteraction::_protein_spring(BaseParticle *p, BaseParticle *q, LR_vector *r, bool update_forces) {
...
number DNANMInteraction::_dna_backbone(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces){
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_bonded_excluded_volume(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_nonbonded_excluded_volume(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_stacking(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_coaxial_stacking(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_hydrogen_bonding(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_cross_stacking(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
...
number DNANMInteraction::_dna_debye_huckel(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(p->btype >= 0 && q->btype >=0){
