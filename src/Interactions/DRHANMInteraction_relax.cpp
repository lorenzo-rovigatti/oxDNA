/*
 * DRHANMInteraction_relax.cpp
 *
 *  Created on: May 25, 2023
 *      Author: Eryk R.
 * 
 * For relaxing initial configurations of systems containing DNA, RNA and protein.
 */
...
DRHANMInteraction_relax::DRHANMInteraction_relax() :
				DRHANMInteraction() {
...
number DRHANMInteraction_relax::_na_backbone(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(!DRHANMInteraction::DNA2Interaction::_check_bonded_neighbour(&p, &q, compute_r)) {
		return (number) 0.f;
	}
...
void DRHANMInteraction_relax::check_input_sanity(std::vector<BaseParticle *> &particles) {
...
void DRHANMInteraction_relax::get_settings(input_file &inp) {
	DRHANMInteraction::get_settings(inp);
