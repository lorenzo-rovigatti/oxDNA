...
void DNA2SDInteraction::init() {
	...
	if(!_average) {
		...
		for(int i = 0; i < 4; i++) {
			for(int j = 0; j < 4; j++) {
				...
				//std::cout << std::setprecision(20) << F1_SHIFT[HYDR_F1][i][j] << std::endl;
				//STACKING
				...
				//std::cout << std::setprecision(20) << F1_SHIFT[STCK_F1][i][j] << std::endl;
				//F2
				...
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_K[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_RC_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_RC[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_R0_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_R0[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_BLOW_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_BLOW[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_BHIGH_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_BHIGH[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_RLOW_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_RLOW[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_RHIGH_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_RHIGH[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_RCLOW_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_RCLOW[CRST_F2][i][j] = tmp_value;
				sprintf(key, "CRST_RCHIGH_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F2_SD_RCHIGH[CRST_F2][i][j] = tmp_value;
				...
				sprintf(key, "CRST_THETA1_A_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_A[CRST_F4_THETA1][i][j] = tmp_value;
				sprintf(key, "CRST_THETA1_B_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_B[CRST_F4_THETA1][i][j] = tmp_value;
				sprintf(key, "CRST_THETA1_T0_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_T0[CRST_F4_THETA1][i][j] = tmp_value;
				sprintf(key, "CRST_THETA1_TS_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TS[CRST_F4_THETA1][i][j] = tmp_value;
				sprintf(key, "CRST_THETA1_TC_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TC[CRST_F4_THETA1][i][j] = tmp_value;
			
				sprintf(key, "CRST_THETA2_A_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_A[CRST_F4_THETA2][i][j] = tmp_value;
				sprintf(key, "CRST_THETA2_B_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_B[CRST_F4_THETA2][i][j] = tmp_value;
				sprintf(key, "CRST_THETA2_T0_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_T0[CRST_F4_THETA2][i][j] = tmp_value;
				sprintf(key, "CRST_THETA2_TS_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TS[CRST_F4_THETA2][i][j] = tmp_value;
				sprintf(key, "CRST_THETA2_TC_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TC[CRST_F4_THETA2][i][j] = tmp_value;
				
				sprintf(key, "CRST_THETA4_A_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_A[CRST_F4_THETA4][i][j] = tmp_value;
				sprintf(key, "CRST_THETA4_B_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_B[CRST_F4_THETA4][i][j] = tmp_value;
				sprintf(key, "CRST_THETA4_T0_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_T0[CRST_F4_THETA4][i][j] = tmp_value;
				sprintf(key, "CRST_THETA4_TS_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TS[CRST_F4_THETA4][i][j] = tmp_value;
				sprintf(key, "CRST_THETA4_TC_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TC[CRST_F4_THETA4][i][j] = tmp_value;
				
				sprintf(key, "CRST_THETA7_A_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_A[CRST_F4_THETA7][i][j] = tmp_value;
				sprintf(key, "CRST_THETA7_B_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_B[CRST_F4_THETA7][i][j] = tmp_value;
				sprintf(key, "CRST_THETA7_T0_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_T0[CRST_F4_THETA7][i][j] = tmp_value;
				sprintf(key, "CRST_THETA7_TS_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TS[CRST_F4_THETA7][i][j] = tmp_value;
				sprintf(key, "CRST_THETA7_TC_%c_%c", Utils::encode_base(i), Utils::encode_base(j));
				if(getInputFloat(&seq_file, key, &tmp_value, 0) == KEY_FOUND) F4_SD_THETA_TC[CRST_F4_THETA7][i][j] = tmp_value;
		...
		for(int i = 0; i < 4; i++) {
			for(int j = 0; j < 4; j++) {			
				...
				//std::cout << "HYDR_THETA4_A_" << Utils::encode_base(i) << "_" << Utils::encode_base(j) << " = " << F4_SD_THETA_A[HYDR_F4_THETA4][i][j] << " " << HYDR_THETA4_A << std::endl;
		...
		for(int i = 0; i < 4; i++) {
			for(int j = 0; j < 4; j++) {			
				...
				//std::cout << "CRST_K_" << Utils::encode_base(i) << "_" << Utils::encode_base(j) << " = " << F2_SD_K[CRST_F2][i][j] << std::endl;				
			}
		}
		//std::cout << "*****************************" << std::endl;
				...
				for(int z = 0; z < 5; z++) {
					...
					if(i != CXST_F4_THETA1) 
						_mesh_f4_SD[i][j][z].build([this](number x, void *args) { return this->_fakef4_SD(x, args); }, [this](number x, void *args) { return _fakef4D_SD(x, args); }, (void*) tmp_args, points, lowlimit, upplimit);
							...
							_mesh_f4_SD[i][j][z].build([this](number x, void *args) { return this->_fakef4_cxst_t1(x, args); }, [this](number x, void *args) { return _fakef4D_cxst_t1(x, args); }, (void*) (&i), points, lowlimit, upplimit);
						}
					assert(lowlimit < upplimit);
				}
			}
	...
	//std::cout << "DNA2SD initialisation done" << std::endl;
	
}


number DNA2SDInteraction::_stacking(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	...
	if(update_forces && energy != (number) 0.f) {
		...
		//std::cout << "STACKING"<< std::endl;
		//std::cout << std::setprecision(20) << f1D << " " << f4t4Dsin << " " << f4t5Dsin << " " << f4t6Dsin << " " <<  f5phi1D << " " << f5phi2D << std::endl;
		...
		//std::cout << p->torque[0] << " " << p->torque[1] << " " << p->torque[2] << " " << q->torque[0] << " " << q->torque[1] << " " << q->torque[2] << std::endl;
		//std::cout << p->force[0] << " " << p->force[1] << " " << p->force[2] << " " << q->force[0] << " " << q->force[1] << " " << q->force[2] << std::endl;
...
number DNA2SDInteraction::_cross_stacking(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(p->is_bonded(q)) {
		return (number) 0.f;
	}
	...
	if(F2_SD_RCLOW[CRST_F2][q->type][p->type]  < rcstackmod && rcstackmod < F2_SD_RCHIGH[CRST_F2][q->type][p->type]) {
		...
		//std::cout << "CSTCK"<< std::endl;
		//std::cout << std::setprecision(16) << " " << rcstackmod << " " << cost1 << " " << cost2 << " " << cost3 << " " << cost4 << " " << cost7 << " " << cost8 << std::endl;

		// functions called at their relevant arguments
		number f2 = _f2_SD(rcstackmod, CRST_F2, q->type, p->type);
		number f4t1 = _custom_f4_SD(cost1, CRST_F4_THETA1, q->type, p->type);
		number f4t2 = _custom_f4_SD(cost2, CRST_F4_THETA2, q->type, p->type);
		number f4t3 = _custom_f4_SD(cost3, CRST_F4_THETA3, q->type, p->type);
		number f4t4 = _custom_f4_SD(cost4, CRST_F4_THETA4, q->type, p->type) + _custom_f4_SD(-cost4, CRST_F4_THETA4, q->type, p->type);
		number f4t7 = _custom_f4_SD(cost7, CRST_F4_THETA7, q->type, p->type) + _custom_f4_SD(-cost7, CRST_F4_THETA7, q->type, p->type);
		number f4t8 = _custom_f4_SD(cost8, CRST_F4_THETA8, q->type, p->type) + _custom_f4_SD(-cost8, CRST_F4_THETA8, q->type, p->type);

		energy = f2 * f4t1 * f4t2 * f4t3 * f4t4 * f4t7 * f4t8;
		
		//std::cout << "cross-stk energy: " << energy << std::endl;
		//std::cout << std::setprecision(16) << f2 << " " << f4t1 << " " << f4t2 << " " << f4t3 << " " << f4t4 << " " << f4t7 << " " << f4t8 << std::endl;

		// makes sense since the above functions can return exactly 0
		if(update_forces && energy != (number) 0.f) {
		
		//std::cout << std::setprecision(16) << p->torque[0] << " " << p->torque[1] << " " << p->torque[2] << " " << q->torque[0] << " " << q->torque[1] << " " << q->torque[2] << std::endl;
		//std::cout << p->force[0] << " " << p->force[1] << " " << p->force[2] << " " << q->force[0] << " " << q->force[1] << " " << q->force[2] << std::endl;
			...
			number f2D = _f2D_SD(rcstackmod, CRST_F2, q->type, p->type);
			number f4t1Dsin = _custom_f4D_SD(cost1, CRST_F4_THETA1, q->type, p->type);
			number f4t2Dsin = _custom_f4D_SD(cost2, CRST_F4_THETA2, q->type, p->type);
			number f4t3Dsin = -_custom_f4D_SD(cost3, CRST_F4_THETA3, q->type, p->type);
			number f4t4Dsin = -_custom_f4D_SD(cost4, CRST_F4_THETA4, q->type, p->type) + _custom_f4D_SD(-cost4, CRST_F4_THETA4, q->type, p->type);
			number f4t7Dsin = _custom_f4D_SD(cost7, CRST_F4_THETA7, q->type, p->type) - _custom_f4D_SD(-cost7, CRST_F4_THETA7, q->type, p->type);
			number f4t8Dsin = -_custom_f4D_SD(cost8, CRST_F4_THETA8, q->type, p->type) + _custom_f4D_SD(-cost8, CRST_F4_THETA8, q->type, p->type);


			//number f2D_o = _f2D(rcstackmod, CRST_F2);
			//number f4t1Dsin_o = _custom_f4D(cost1, CRST_F4_THETA1);
			//number f4t2Dsin_o = _custom_f4D(cost2, CRST_F4_THETA2);
			//number f4t3Dsin_o = -_custom_f4D(cost3, CRST_F4_THETA3);
			//number f4t4Dsin_o = -_custom_f4D(cost4, CRST_F4_THETA4) + _custom_f4D(-cost4, CRST_F4_THETA4);
			//number f4t7Dsin_o = _custom_f4D(cost7, CRST_F4_THETA7) - _custom_f4D(-cost7, CRST_F4_THETA7);
			//number f4t8Dsin_o = -_custom_f4D(cost8, CRST_F4_THETA8) + _custom_f4D(-cost8, CRST_F4_THETA8);

			//std::cout << std::setprecision(16) << F4_SD_THETA_A[CRST_F4_THETA8][q->type][p->type] << " " << F4_SD_THETA_B[CRST_F4_THETA8][q->type][p->type] << " " << F4_SD_THETA_T0[CRST_F4_THETA8][q->type][p->type] << " " << F4_SD_THETA_TS[CRST_F4_THETA8][q->type][p->type] << " " <<  F4_SD_THETA_TC[CRST_F4_THETA8][q->type][p->type] << std::endl;
			//std::cout << std::setprecision(16) << f2D-f2D_o << " " << f4t1Dsin-f4t1Dsin_o << " " << f4t2Dsin-f4t2Dsin_o << " " << f4t3Dsin-f4t3Dsin_o << " " <<  f4t4Dsin-f4t4Dsin_o << " " << f4t7Dsin-f4t7Dsin_o << " " << f4t8Dsin-f4t8Dsin_o << std::endl;
			
			//std::cout << std::setprecision(16) << f2D_o << " " << f4t1Dsin_o << " " << f4t2Dsin_o << " " << f4t3Dsin_o << " " <<  f4t4Dsin_o << " " << f4t7Dsin_o << " " << f4t8Dsin_o << std::endl;
			
			//std::cout << std::setprecision(16) << f2D << " " << f4t1Dsin << " " << f4t2Dsin << " " << f4t3Dsin << " " <<  f4t4Dsin << " " << f4t7Dsin << " " << f4t8Dsin << std::endl;
			
			// RADIAL PART
			force = -rcstackdir * (f2D * f4t1 * f4t2 * f4t3 * f4t4 * f4t7 * f4t8);
			
			/*
			std::cout << rcstackdir << std::endl;
			number tttt = f2D * f4t1 * f4t2 * f4t3 * f4t4 * f4t7 * f4t8;
			std::cout << std::setprecision(16) << "tttt: " << tttt << std::endl;
			std::cout << -rcstackdir[0]* tttt << " " <<  -rcstackdir[1]* tttt << " " << -rcstackdir[2]* tttt << std::endl;
			std::cout << std::setprecision(16) << "1 -- " << force << std::endl;
			*/
...
number DNA2SDInteraction::_hydrogen_bonding(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(p->is_bonded(q)) {
		return (number) 0.f;
	}
	...
	number energy = (number) 0.f;
	//if(q->type == 4 || p->type == 4) std::cout << "There's a ghost!" << std::endl;
	...
	//if(is_pair && HYDR_RCLOW < rhydromod && rhydromod < HYDR_RCHIGH) {
		...
		if(update_forces && energy != 0.) {
			...
			//std::cout << "HYDRO"<< std::endl;
			//std::cout << std::setprecision(20) << f1D << " " << f4t1Dsin << " " << f4t2Dsin << " " << f4t3Dsin << " " <<  f4t4Dsin << " " << f4t7Dsin << " " << f4t8Dsin << std::endl;
...
number DNA2SDInteraction::_f1_SD(number r, int type, int n3, int n5) {
	number val = (number) 0;
...
number DNA2SDInteraction::_f1D_SD(number r, int type, int n3, int n5) {
	number val = (number) 0;
	if(r < F1_SD_RCHIGH[type][n3][n5]) {
		if(r > F1_SD_RHIGH[type][n3][n5]) {
...
number DNA2SDInteraction::_f2_SD(number r, int type, int n3, int n5) {
	number val = (number) 0.;
...
number DNA2SDInteraction::_f2D_SD(number r, int type, int n3, int n5) {
	number val = (number) 0.;
	if(r < F2_SD_RCHIGH[type][n3][n5]) {
		if(r > F2_SD_RHIGH[type][n3][n5]) {
...
number DNA2SDInteraction::_fakef4_SD(number t, void *par) {
...
number DNA2SDInteraction::_fakef4D_SD(number t, void *par) {
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1.0001))
		throw oxDNAException("In function DNAInteraction::_fakef4() t was found to be out of the range [-1,1] by a large amount, t = %g", t);
	if((*(int*) par == CXST_F4_THETA1) && (t * t > 1))
		t = (number) copysign(1, t);
...
number DNA2SDInteraction::_f4_SD(number t, int type, int n3, int n5) {
	number val = (number) 0;
...
number DNA2SDInteraction::_f4D_SD(number t, int type, int n3, int n5) {
	number val = (number) 0;
	number m = (number) 1;
	t -= F4_SD_THETA_T0[type][n3][n5];
...
number DNA2SDInteraction::_f4Dsin_SD(number t, int type, int n3, int n5) {
	number val = (number) 0;
	number m = (number) 1;
...
number DNA2SDInteraction::_f5_SD(number f, int type, int n3, int n5) {
	number val = (number) 0;
...
number DNA2SDInteraction::_f5D_SD(number f, int type, int n3, int n5) {
	number val = (number) 0;
