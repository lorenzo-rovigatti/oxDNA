...
void Timer::resume () {
	if (_active ) throw oxDNAException ("Error: resuming already active timer %s", _desc.c_str());
	_last = clock();
	_active = true;
}

void Timer::pause () {
	if (!_active ) throw oxDNAException ("Error: pausing resuming already inactive timer %s", _desc.c_str());
	_time += (clock() - _last);
	_active = false;
}
...
long long int Timer::get_time () {
	if (_active) return (long long int) (_time + (clock() - _last));
...
void TimingManager::init () {
	if (_timingManager != NULL) throw oxDNAException ("initializing alrady initialized TimingManager");
...
void TimingManager::print () {
	...
	for (unsigned int i = 0; i < mylist.size(); i ++) {
		...
		OX_LOG (Logger::LOG_INFO, "%-30s %12.3lf %12.3lf %12.3lf", (char *) mystr, totaltimes[t] / CPSF, own_time[t] / CPSF, sum_of_children[t] / CPSF);
