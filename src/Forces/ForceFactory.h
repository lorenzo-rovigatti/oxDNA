...
public:
	 ...
	 */
	void add_force(input_file &inp, std::vector<BaseParticle *> &particles, int N, bool is_CUDA, BaseBox * box_ptr);

	/// adds forces. Used by SimBackend and GeneratorManager
	void read_external_forces(std::string external_filename, std::vector<BaseParticle *> & particles, int N, bool is_CUDA, BaseBox * box_ptr);
};
