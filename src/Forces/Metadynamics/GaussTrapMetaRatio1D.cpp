...
GaussTrapMetaRatio1D::GaussTrapMetaRatio1D() :
				BaseForce() {
	xmin = 0;
	xmax = 10;
	N_grid = 0;
...
std::tuple<std::vector<int>, std::string> GaussTrapMetaRatio1D::init(input_file &inp) {
...
LR_vector GaussTrapMetaRatio1D::_distance(LR_vector u, LR_vector v) {
	if(PBC)
