...
BinVerletList::BinVerletList(int &N, BaseBox *box) : BaseList(N, box), _updated(false), _is_AO(false) {

}


BinVerletList::~BinVerletList() {
	for(typename vector<Cells *>::iterator it = _cells.begin(); it != _cells.end(); it++) delete *it;
}
...
void BinVerletList::init(BaseParticle **particles, number rcut) {
	...
	for(int i = 0; i < 3; i++) {
		...
		Cells *cn = new Cells(this->_N, this->_box);
