...
void SaltExtrapolation::get_settings(input_file &my_inp, input_file &sim_inp) {
	...
	for(unsigned int i = 0; i < salt_strings.size(); i++)
		_salts.push_back((number) atof(salt_strings[i].c_str()));
...
std::string SaltExtrapolation::get_output_string(llint curr_step) {
	...
	for(i = 0; i < _temps.size(); i++)
		for(j = 0; j < _salts.size(); j++)
			_interactions[i][j]->set_box(_config_info->box);
	...
	for(j = 0; j < _salts.size(); j++) {
		...
		for(i = 0; i < _temps.size(); i++)
			output_str += Utils::sformat("%g ", _temps[i]);
		...
		for(k = 0; k < _hists[0][j].size(); k++) { // k runs over the op states
			...
			for(i = 0; i < _temps.size(); i++)
				output_str += Utils::sformat("%Le ", _hists[i][j][k]); // j runs over the temperatures
			output_str += Utils::sformat("\n");
		}
	}
	output_str += Utils::sformat("\n");
