...
__global__ void lj_forces(number4 *poss, number4 *forces, CUDABox*box) {
...
__global__ void lj_forces_edge(number4 *poss, number4 *forces, edge_bond *edge_list, int n_edges, CUDABox*box) {
	if(IND >= n_edges) return;

	number4 dF = make_number4(0.f, 0.f, 0.f, 0.f);
...
__global__ void lj_forces(number4 *poss, number4 *forces, int *matrix_neighs, int *number_neighs, CUDABox*box) {
	if(IND >= MD_N[0]) return;
