...
__global__ void sum_edge_forces_torques(number4 *edge_forces, number4 *forces, number4 *edge_torques, number4 *torques, int N, int n_forces) {
	...
	for(int i = 0; i < n_forces; i++) {
		...
		edge_forces[N * i + IND] = make_number4(0, 0, 0, 0);
		...
		edge_torques[N * i + IND] = make_number4(0, 0, 0, 0);
	}
...
void CUDABaseInteraction::_sum_edge_forces(number4 *d_forces) {
	sum_edge_forces
...
void CUDABaseInteraction::_sum_edge_forces_torques(number4 *d_forces, number4 *d_torques) {
...
void CUDABaseInteraction::cuda_init(number box_side, int N) {
	_N = N;
	_box_side = box_side;
...
void CUDABaseInteraction::_hb_op_precalc(number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, float *hb_energies, int n_threads, bool *region_is_nearhb, CUDA_kernel_cfg _ffs_hb_precalc_kernel_cfg, CUDABox*d_box) {
	throw oxDNAException("On CUDA, FFS is only implemented for the DNA and RNA interactions");
}

void CUDABaseInteraction::_near_hb_op_precalc(number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, bool *nearly_bonded_array, int n_threads, bool *region_is_nearhb, CUDA_kernel_cfg _ffs_hb_precalc_kernel_cfg, CUDABox*d_box) {
	throw oxDNAException("On CUDA, FFS is only implemented for the DNA and RNA interactions");
}

void CUDABaseInteraction::_dist_op_precalc(number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, number *op_dists, int n_threads, CUDA_kernel_cfg _ffs_dist_precalc_kernel_cfg, CUDABox*d_box) {
	throw oxDNAException("On CUDA, FFS is only implemented for the DNA and RNA interactions");
}
