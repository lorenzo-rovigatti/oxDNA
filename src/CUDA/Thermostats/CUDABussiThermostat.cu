...
struct compute_K {
	__device__
	tmpnmbr operator()(const tmpnmbr& a) const {
		tmpnmbr res;
		res.x = res.y = res.z = 0;
...
__global__ void bussi_thermostat(tmpnmbr *vels, tmpnmbr *Ls, c_number rescale_factor_t, c_number rescale_factor_r, int N) {
	if(IND >= N) return;
...
void CUDABussiThermostat::apply_cuda(tmpnmbr *d_poss, GPU_quat *d_orientations, tmpnmbr *d_vels, tmpnmbr *d_Ls, llint curr_step) {
	if(!would_activate(curr_step)) return;

	// we first calculate the current kinetic energy
	thrust::device_ptr<tmpnmbr> t_vels = thrust::device_pointer_cast(d_vels);
	thrust::device_ptr<tmpnmbr> t_Ls = thrust::device_pointer_cast(d_Ls);

	tmpnmbr zero = { 0., 0., 0., 0. };
	tmpnmbr K_now_t = thrust::transform_reduce(t_vels, t_vels + this->_N_part, compute_K(), zero, sum_K());
	tmpnmbr K_now_r = thrust::transform_reduce(t_Ls, t_Ls + this->_N_part, compute_K(), zero, sum_K());
