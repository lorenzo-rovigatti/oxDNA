...
void VMMC_CPUBackend::init() {
	...
	if(_have_us) {
		_op.init_from_file(_op_file, _particles, this->_N);
...
inline void VMMC_CPUBackend::_move_particle(movestr *moveptr, BaseParticle *q, BaseParticle *p) {
	if(moveptr->type == MC_MOVE_TRANSLATION) {
		...
		if(p->strand_id == q->strand_id) {
			...
			dr = this->_box->min_image(p_old->pos, q->pos);
		}
...
void VMMC_CPUBackend::sim_step(llint curr_step) {
	...
	for(int i = 0; i < this->_N; i++) {
		...
		if(move.type == MC_MOVE_TRANSLATION) {
			move.t = LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian()) * this->_delta[MC_MOVE_TRANSLATION];
			...
			//pp = &(this->_particles[clust[0]]);
			move.R = Utils::get_random_rotation_matrix_from_angle(this->_delta[MC_MOVE_ROTATION] * Utils::gaussian());
...
void VMMC_CPUBackend::_print_pos(int id) {
	...
	printf("%5i - np.array([% 10.6e % 10.6e % 10.6e])\n        np.array([% 10.6e % 10.6e % 10.6e])\n        np.array([% 10.6e % 10.6e % 10.6e])\n        np.array([% 10.6e % 10.6e % 10.6e])\n", id, p->pos.x, p->pos.y, p->pos.z, p->orientationT.v1.x, p->orientationT.v1.y, p->orientationT.v1.z, p->orientationT.v2.x, p->orientationT.v2.y, p->orientationT.v2.z, p->orientationT.v3.x, p->orientationT.v3.y, p->orientationT.v3.z);
	return;
}
