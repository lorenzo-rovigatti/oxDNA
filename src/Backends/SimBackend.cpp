...
bool SimBackend::read_stored_configuration(std::string conf,std::string box) {

	std::string delim = " ";
	std::vector<double> box_d;
	box_d = split_string(box,delim);
	std::vector<double> conf_d;
	conf_d = split_string(conf,delim);
	
	double Lx=box_d[0];
	double Ly=box_d[1];
	double Lz=box_d[2];	//(box) check if needed
	// parse headers. Binary and ascii configurations have different headers, and hence
	// we have to separate the two procedures
	...
	*/
	...
	while(i < N()) {
		...
		p->pos = LR_vector(conf_d[i*15+0], conf_d[i*15+1], conf_d[i*15+2]);
		p->orientation.v1 = LR_vector(conf_d[i*15+3], conf_d[i*15+4], conf_d[i*15+5]);
		p->orientation.v3 = LR_vector(conf_d[i*15+6], conf_d[i*15+7], conf_d[i*15+8]);
		...
		// read the momenta
		p->vel = LR_vector(conf_d[i*15+9], conf_d[i*15+10], conf_d[i*15+11]);
		p->L = LR_vector(conf_d[i*15+12], conf_d[i*15+13], conf_d[i*15+14]);
...
void SimBackend::update_interaction_parameters(std::string names, std::string values) {
	_interaction->update_parameters(names, values);
}
